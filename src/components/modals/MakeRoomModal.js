...
const MakeRoomModal = (props) => {
  ...
  const startTimeList = ['1분 뒤', '50분 뒤', '150분 뒤'];
  // 태그선택
  ...
  const clickMakeBtn = () => {
    ...
    const checkInputArray = [$RoomNameInput.current.value, difficultyList[clickedDifficulty], dropdownList[clickedCategory], $LinkInput.current.value, startTimeList[clickedStartTime]];
            ...
            {startTimeList.map((e, i) => (
              <DB_EL
                ...
                style={{
                  ...
                  background: clickedStartTime === i ? '#222529' : '',
                }}
              >
                {e}
              </DB_EL>
