...
const RecommendList = () => {
  ...
  const getCardInfo = async (videoUrl) => {
    let videoId = _parserVideoId(videoUrl);
    return await _getVideoInfo(videoId).then((e) => e);
  };
    ...
    <RecommendListContainer>
      ...
      <RecommendCards>
        {suggestionsArray.map((e, i) => {
          const data = getCardInfo(e);
          console.log(data);
          return <RecommendCard key={i}></RecommendCard>;
        })}
      </RecommendCards>
    </RecommendListContainer>
  );
